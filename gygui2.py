...
class GyroidGeneratorGUI(QMainWindow):
    def __init__(self):
        ...
        # Color selection
        color_button1 = QPushButton("Choose Primary Color")
        color_button1.clicked.connect(self.choose_primary_color)
        left_layout.addWidget(color_button1)
        self.color_descriptor1 = QLabel("Primary Color: Click to choose gyroid color")
        left_layout.addWidget(self.color_descriptor1)
        self.gyroid_color1 = [1, 1, 1]  # Default color: white

        color_button2 = QPushButton("Choose Secondary Color")
        color_button2.clicked.connect(self.choose_secondary_color)
        left_layout.addWidget(color_button2)
        self.color_descriptor2 = QLabel("Secondary Color: Click to choose gyroid color")
        left_layout.addWidget(self.color_descriptor2)
        self.gyroid_color2 = [0, 0, 0]  # Default color: black
    ...
    def generate_gyroid(self):
        try:
            ...
            # Visualize with dual colors
            self.plotter.clear()
            self.plotter.add_mesh(self.gyroid_mesh, scalars=self.gyroid_mesh.points[:, -1], show_scalar_bar=False)
            self.plotter.add_mesh(self.gyroid_mesh, color=self.gyroid_color1, scalars=fun_values, show_scalar_bar=False)
            self.plotter.add_mesh(self.gyroid_mesh, color=self.gyroid_color2, scalars=-fun_values, show_scalar_bar=False)
    ...
    def choose_primary_color(self):
        ...
        if color.isValid():
            self.gyroid_color1 = [color.red() / 255, color.green() / 255, color.blue() / 255]
            self.color_descriptor1.setText(f"Primary Color: {color.name()}")

    def choose_secondary_color(self):
        color = QColorDialog.getColor()
        if color.isValid():
            self.gyroid_color2 = [color.red() / 255, color.green() / 255, color.blue() / 255]
            self.color_descriptor2.setText(f"Secondary Color: {color.name()}")
