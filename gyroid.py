...
class GyroidGeneratorGUI(QMainWindow):
    ...
    def save_mesh(self):
        ...
        # Allow the user to choose between STL or OBJ
        ...
        if filename:
            try:
                if filetype == "OBJ Files (*.obj)":
                    ...
                    self.gyroid_mesh.save(filename)  # Save as STL
    ...
    def save_as_obj(self, filename):
        ...
        mesh = trimesh.Trimesh(vertices=self.gyroid_mesh.points, faces=self.gyroid_mesh.faces.reshape((-1, 4))[:, 1:])
        mesh.export(filename)  # Save as OBJ
